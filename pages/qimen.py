...
# ==========================================================
# 模組 8: 👁️ 人心判斷系統 (詳細版) - [NEW]
# ==========================================================
elif mode == "👁️ 人心判斷系統 (詳細版)":
    st.markdown("## 👁️ 紅蓮・人心判斷系統")
    st.info("輸入生日，透過八字日主看穿對方的底層邏輯與使用說明書。")

    col1, col2 = st.columns(2)
    with col1:
        target_name = st.text_input("對象代號", "神秘人")
        target_birthday = st.date_input("對象生日", datetime.date(1996, 2, 17))
    with col2:
        analysis_mode = st.radio("啟動模式", ["交友/看透人心", "面試/識人用人"])

    if st.button("🔥 啟動紅蓮解析"):
        profile = get_human_profile(target_birthday, analysis_mode)
        ...
        m1.metric("日主核心", profile['day_master'])
        m2.metric("星座", profile['constellation'])
        m3.metric("生肖", profile['zodiac'])
        m4.metric("農曆", profile['lunar_date'][5:]) # 簡化顯示
        
        # 詳細分析區塊
        st.markdown(f"""
        <div class="human-card">
            <h3 style="color:#00e5ff; margin-top:0;">📝 {analysis_mode} 報告</h3>
            <div style="font-size:16px; line-height:1.6;">{profile['text']}</div>
        </div>
        """, unsafe_allow_html=True)
            
        # 戰略錦囊
        st.markdown(f"""
        <div class="strategy-box" style="border-left-color:#FFD700;">
            <div style="font-weight:bold; color:#FFD700; margin-bottom:5px;">🔱 紅蓮副官指令：</div>
            {profile['strategy']}
        </div>
        """, unsafe_allow_html=True)
