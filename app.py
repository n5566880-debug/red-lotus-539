...
for d, n in zip(reversed_dates, reversed_nums):
    ...
    st.sidebar.markdown("---")
st.sidebar.caption("⚡ 賭神級運算模組 v6.3")
...
with f_col1:
    ...
    <div class="firepower-card-sat">
        ...
        <p style="font-size:12px; color:#aaa;">基於乖離率 -28% 與真空回補邏輯運算</p>
    </div>
    """, unsafe_allow_html=True)
with f_col2:
    st.markdown("""
    <div class="firepower-card-pre">
        ...
        <p style="font-size:12px; color:#aaa;">基於生日磁場與中軸引力回歸運算</p>
    </div>
    """, unsafe_allow_html=True)
...
with tab1:
    ...
    with col_b2:
        st.info("💡 負乖離越大，代表回歸 100 中軸的力道越強。")
    fig_trend = go.Figure()
