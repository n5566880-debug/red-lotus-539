...
def calculate_witch_chaos():
    ...
    # 更新：配合今日 24/25 追擊策略
    target_num = 25
    reason = "34(火)已開，連號慣性啟動，鎖定『25(晴)』填補空缺。"
    return target_num, reason, now.strftime("%H:%M:%S")

def get_pressure_data():
    # 更新：1/9 最新壓力數據 (24極危，25蓄能)
    return [
        (24, 33, "level-critical", "#FF0000", "🚨 極危 (鄰號修正)"),
        (25, 19, "level-warning", "#FFD700", "⚠️ 蓄能 (連號效應)"),
        (34, 0, "level-safe", "#32CD32", "🟢 釋放 (已開出)"), # 34 剛開，壓力釋放
        (7, 12, "level-warning", "#FFD700", "⚠️ 蓄能 (同源)"),
        (11, 8, "level-safe", "#32CD32", "🟢 平穩"),
        (22, 0, "level-safe", "#32CD32", "🟢 釋放 (已開出)")
    ]

# --- 3. 數據準備 (更新至 1/9) ---
data = {
    '日期': ['1/5', '1/6', '1/7', '1/8', '1/9'],
    '開出號碼': [
        [15, 23, 32, 36, 39], 
        [4, 13, 21, 28, 35], 
        [5, 10, 14, 15, 28], 
        [3, 8, 10, 21, 30],
        [1, 12, 14, 22, 34]  # 1/9 最新開獎
    ]
}
...
for p in ["寧棄莫出銃", "人旺我亂碰", "牌衰過三棟", "牌尾吃卡檔", "牌品不可輸", "贏的是人不是牌", "牌弱打生死張", "人賤天收"]:
...
c1.info("氣象：火旺 (34已開)")
c2.success("財神：正西 (利金)")
c3.warning("能量：24/25 追擊中")

st.markdown("---")

# === 第一層：深紅警戒壓力表 (The Visual Trigger) ===
st.markdown("### 🩸 回彈臨界點 (Pressure Gauge)")
st.caption("魔女直覺區：看到紅色呼吸燈，請立即關注")
...
# 更新：針對 1/10 的最新預測號碼
strategy_nums = [7, 11, 24, 25, 35] 

with col_main:
    st.markdown('<div class="action-card">', unsafe_allow_html=True)
    st.markdown('<h3 style="color:#FFD700;">⚔️ 統帥・五星總攻 (1/10)</h3>', unsafe_allow_html=True)
